 ...
 * yarn localize -- -h
 *
 * yarn localize -- -e vim -u
 * ```
 *
 * Утилита `localize` автоматически прочтет документацию функций и локализацию,
 * найдет различия в документации и обновит локализацию.
 *
 * Для адаптации к другому проекту необходимо скопировать файл `/utils/uodate-loc.js` в свой проект.
 *
 * При использовании утилиты можно настроить пути к локализации, текстовый редактор и другие параметры.
 *
 * Для того, чтобы утилита нашла локализацию, необходимо соблюсти следующую конструкцию:
 *
 * ```
 * LOC = {
 * //#region CTX-NAME
 ...
 * ```
 *
 * Комментарии регионов должны начинаться в начале строки.
 * Вся область внутри региона будет заменена на актуальную локализацию.
 */
...
//#region cons
    "cons": {
      en: `/**
            ...
            * @category Working with Arrays | 23
            */`,
      ru: `/**
